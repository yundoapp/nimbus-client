...
class _DevicesContent extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Column(
      ...
      children: [
        Text('${items.length}/${limit == 0 ? '--' : limit} 台设备'),
...
class _DeviceTile extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return ListTile(
      ...
      title: Row(
        children: [
          Expanded(child: Text(device.deviceName.isEmpty ? '未知设备' : device.deviceName)),
          if (device.isCurrent)
            Container(
      ...
      subtitle: Text(
        '${_platformName(device.platform)} · ${device.appVersion} · ${_lastActiveText(device.lastActiveAt)}',
      ),
  ...
  final confirmed =
      await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          ...
          content: Text('确定删除“${device.deviceName.isEmpty ? '未知设备' : device.deviceName}”吗？'),
          actions: [
...
String _lastActiveText(DateTime? value) {
  if (value == null) return '最近活跃 --';
  return '最近活跃 ${value.year}-${_twoDigits(value.month)}-${_twoDigits(value.day)} ${_twoDigits(value.hour)}:${_twoDigits(value.minute)}';
}

                      ...
                      _In_ wchar_t *command_line, _In_ int show_command)
{
  ...
  if (SendAppLinkToInstance(L"Yundo"))
  {
    return EXIT_SUCCESS;
  }

  HANDLE hMutexInstance = CreateMutex(NULL, TRUE, L"YundoMutex");
  HWND handle = FindWindowA(NULL, "Yundo");
  ...
  if (GetLastError() == ERROR_ALREADY_EXISTS)
  {
    ...
    if (window.SendAppLinkToInstance(L"Yundo"))
    {
      return false;
    }
  ...
  if (!window.Create(L"Yundo", origin, size))
  {
    return EXIT_FAILURE;
  }

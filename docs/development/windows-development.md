...
M4 Windows MVP 已进入实体 x64 Windows 总验收。Parallels Windows 11 ARM64 已用于早期安装和品牌复验；从 `1.0.0+10004` 起，安装器只允许原生 x64 Windows。2026-07-19 的首轮实体 Windows 11 Pro 验收确认安装、注册、登录和激活通过，同时暴露普通用户启动失败与旧版错误不可诊断的阻断项，转由平台 Issue #88 修复并复测。
...
从 `1.0.3+10008` 起，Windows 启动错误不再统一折叠为“稍后重试”：
...
旧版本只向平台上报 `CLIENT_START_FAILED`，且 UI 丢弃了原始系统错误，因此无法仅凭旧截图确认具体失败类别。安装 `1.0.3+10008` 后复测一次即可依据诊断编号继续定位；这也是实体 Windows #81/#88 的下一步验收依据。

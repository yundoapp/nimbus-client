...
1. 普通 core 配置移除 TUN 和产品级 `route/reject/rule_set` 决策，保留本机 mixed 入站、真实出站以及 Hiddify 原生 `route.final`；只允许必要的 sniff/DNS 控制动作继续服务本机入站。
2. 特权配置只保留 TUN、本机 SOCKS、direct 和从完整配置脱敏投影的有效规则顺序，由 Helper 单独决定直连、拦截或送入普通 core。
...
- `reject`、`sniff` 和必要的 DNS 控制动作按白名单投影；普通 core 不再重复执行产品级直连、拦截或规则库匹配，但会保留 Hiddify DNS 规则所引用的原始规则库定义，避免 DNS 初始化缺少 tag。
- 未命中规则的流量最终进入普通 core；普通 core 保持 Hiddify 原生主出站，不再由云渡扩展字段改写最终出站。
- 规则模式不再硬编码 UDP 全量加速；TCP、UDP 和其他网络类型都按同一套用户规则、通用规则和 Helper 最终出口判断。全局模式使用有匹配条件的 IPv4/IPv6 规则在 Helper 层覆盖全部公网流量。

投影需要的远程二进制规则集只允许 HTTPS、不得携带 URL 凭据，并固定通过本机 SOCKS 下载；每个规则库同时携带签名 App 内的同名 SRS 作为首次启动兜底。Helper 只接收域名/IP/端口/进程等匹配条件、规则集 URL 和受校验的包内路径，不接收真实节点参数；活动配置位于 root 私有目录，停止后删除，不保留配置副本。
...
2. App 复用 Helper 将要使用的同一 SOCKS5 地址、端口和认证信息，并发探测固定 IPv4/IPv6 公网地址：双通道均成功时保留双栈；IPv4 成功但 IPv6 失败时，仅把本次会话重启为 IPv4 兜底配置；IPv4 失败时停止普通 core 并返回失败。
...
IPv4/IPv6 探测使用 SOCKS5 CONNECT 和字面量 HTTP 地址，默认最多尝试 2 次、每次 1.5 秒并间隔 200ms，不依赖尚未接管的系统 DNS、TLS 证书或 HTTP CONNECT 代理行为。探测路径与 Helper 的真实数据路径一致，只判断当前加速出站能否建立对应地址族的连接，不缓存节点结论；每次开始或重建加速都会重新判定。双栈都可用时保留双栈，只有 IPv4 可用时才进入 IPv4 兜底；IPv4 探测仍失败时停止普通 core 并拒绝启动 Helper/TUN，不接管系统网络。

2026-08-06 曾发现旧探测通过 Dart `HttpClient` 的 HTTP 代理接口访问 HTTPS 字面量地址，而 Helper 实际通过 SOCKS5 转发。部分节点可正常承载 SOCKS5 IPv4，却会重置该 HTTP CONNECT/TLS 探测，造成 `Y-NETWORK-001` 误报。现已统一为真实 SOCKS5 数据路径，并通过本地协议级测试覆盖“IPv4 可用、IPv6 不可用时进入 IPv4 兜底”的场景。
...
- 主动断开：先停止普通 core，再通过 XPC 停止 TUN；Helper 对当前 worker 先发 TERM，有界等待后才使用 KILL 清理自己的残留进程。
- 主动断开期间首页保持“正在停止加速”并禁用重复点击，至少展示 500ms；Helper 必须确认 worker、活动配置、固定 TUN 地址、系统路由和 DNS 状态均已释放，App 再复核云渡自有路由为 0，之后才恢复“加速”。任一复核失败都会保留停止失败状态并写入诊断日志。服务端会话上报为尽力而为的后台步骤，不阻塞本机停止状态。
...
客户端构建前由 `scripts/cache_yundo_rule_sets.sh` 刷新 `assets/rules/`。远程公共规则包
可用时，按平台发布的版本、来源指纹和 `sourceUrl` 下载全部规则；远程接口暂时不可用时，
使用仓库中上一份带 SHA-256 的可信清单重新下载和校验全部规则。两种路径都要求全部下载
成功后才原子替换，避免把半套规则写进 App；正式包的来源信息始终保留云渡公共规则接口，
不会把本地 API 地址写入用户包。
...
2026-08-04 曾为 macOS 加速启动阶段增加网络能力有限重试：当时 IPv4/IPv6 探测各自最多尝试 5 次、每次间隔 1 秒。该历史方案已被上文当前 SOCKS5 探测取代；现行实现最多尝试 2 次、每次 1.5 秒并间隔 200ms，文档与运行时以“首次授权流程”一节为准。

2026-08-05 排查反复启停的间歇性失败：发现 App 恢复前台时会重新执行 core setup，旧逻辑会删除正在等待网络探测的 `yundo-user-core-ipv4-fallback.json`，随后用户 core 重启收到 `no such file or directory`。现已取消 setup 阶段对活动临时配置的清理，并在探测前校验、必要时从内存恢复 IPv4 兜底配置。当时版本曾在短时 IPv4 探测失败后继续使用 IPv4 兜底；该行为已于 2026-08-06 被失败关闭策略取代，当前只有 IPv4 明确可用、IPv6 不可用时才允许兜底，IPv4 失败时不得启动 Helper/TUN。开发版首页增加的“加速过程”入口后来统一迁移到设置页“诊断日志”。
...
2026-08-05 修复正式版停止后再次启动的间歇性失败：macOS 在切换 IPv4 兜底配置时，先停止旧的 Core 状态/日志流，再重新绑定本地 gRPC 客户端；Core 重启期间出现的预期 HTTP/2 stream termination 会等待核心恢复到 `STARTED` 后继续监听，不再直接归类为 `Y-CORE-003`。每次启动前也会刷新本地 gRPC 通道，避免复用上一次停止留下的半关闭连接；旧通道关闭最多等待 1 秒，避免启动流程被半关闭连接卡住。macOS Dock 状态栏不再展示延时文字或红色延时徽章；构建号 `202608103` 用于本轮 macOS 验证。

2026-08-07 增加可控的精确路由历史：Helper 内 Core 是 macOS 产品路由的唯一事实来源，通过显式 `/connections?yundo_exact_history=1` 返回 TCP/UDP 的加速、直连、拒绝三态和最近关闭的短连接。Flutter 不再同时监听用户 Core，也不根据中间链推断。普通 `/connections` 行为未改变；编译时可用 `YUNDO_EXACT_ROUTE_HISTORY=false` 关闭精确通道，底层实现集中在独立 `0004-exact-route-decision-history.patch`，便于回退和同步上游。

...
2026-08-05 修复正式版停止后再次启动的间歇性失败：macOS 在切换 IPv4 兜底配置时，先停止旧的 Core 状态/日志流，再重新绑定本地 gRPC 客户端；Core 重启期间出现的预期 HTTP/2 stream termination 会等待核心恢复到 `STARTED` 后继续监听，不再直接归类为 `Y-CORE-003`。每次启动前也会刷新本地 gRPC 通道，避免复用上一次停止留下的半关闭连接。macOS Dock 状态栏不再展示延时文字或红色延时徽章；构建号 `202608102` 用于本轮 macOS 验证。
